...
<template>
  <section class="px-4 py-10">
    <div class="container-xl lg:container m-auto">
      <h2 class="text-3xl font-bold text-theme mb-6 text-center">Browse Jobs</h2>
      <!-- Show loading spinner -->
      <div v-if="jobsState.isLoading" class="text-center text-grey-500 py-6"><PulseLoader /></div>
      <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <JobCard v-for="job in jobsState.jobs.slice(0, props.limit || jobsState.jobs.length)" :key="job.id" :job="job">
          {{ job.title }}
        </JobCard>
      </div>
    </div>
